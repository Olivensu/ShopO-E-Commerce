...
const FlashSaleTable = () => {
    ...
    const deleteUsers=(id)=>{
        axios.delete(`${import.meta.env.VITE_BACKEND_URL}/api/event/flash-sale/${id}`)
        .then(res=>{
            console.log(res);
            axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/event/flash-sale`)
            .then(res=>{
      ...
      <div>
        ...
        <div className="relative float-left w-[calc(100%-200px)] max-h-screen overflow-auto  bg-slate-50">
            <div className='float-right mr-5'>
                <Link to='/dashboard/upload-flash-sale'><button className='btn btn-primary text-white my-3 btn-sm'>Add Picture</button></Link>
            </div>
