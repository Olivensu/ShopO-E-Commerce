...
// console.log(userinfo);
    ...
    const orderedItem = orderItem?.filter((item, index) => item.status==='PickupPoint')
      

    const handleSubmit=(id)=>{

        axios.put(`${import.meta.env.VITE_BACKEND_URL}/api/order/status`,{_id:id, status: 'Delivered'})
        .then(res=>{
            axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/order/`)
            .then(res=>{
                console.log(res.data.payload);
    ...
    const handleCancelled=(id)=>{
        ...
        axios.put(`${import.meta.env.VITE_BACKEND_URL}/api/order/status`,{_id:id, status: 'Cancelled'})
        .then(res=>{
            axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/order/`)
            .then(res=>{
                console.log(res.data.payload);
              ...
              <p className="text-xl">No Order Here...</p>
                ...
                >
                  ...
                  <div className="grid justify-items-center grid-cols-3 gap-5 mt-5">
                    ...
                    <button  onClick={()=>handleCancelled(item._id)} className='btn-sm  ml-4 btn-primary rounded-xl'>Cancelled</button>
                    </div>
                  </div>
                </div>
              ))
